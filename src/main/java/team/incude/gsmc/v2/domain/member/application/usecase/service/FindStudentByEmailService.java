 ...
 * <p>{@link FindStudentByEmailUseCase}를 구현하며, 주어진 학생 코드에 해당하는 학생 정보를 조회하여 {@link GetStudentResponse}로 반환합니다.
 * <p>{@link StudentDetailPersistencePort}를 통해 학생 상세 정보와 증빙 상태를 함께 조회합니다.
 * @author snowykte0426
 */
@Service
@RequiredArgsConstructor
public class FindStudentByEmailService implements FindStudentByEmailUseCase {

    private final StudentDetailPersistencePort studentDetailPersistencePort;

    @Override
    public GetStudentResponse execute(String email) {
        StudentDetailWithEvidence studentDetailWithEvidence = studentDetailPersistencePort.findStudentDetailWithEvidenceByMemberEmail(email);
