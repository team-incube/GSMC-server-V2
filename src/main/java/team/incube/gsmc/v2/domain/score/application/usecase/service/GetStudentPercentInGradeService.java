...
/**
 * 학생의 성적 백분위수를 학년 단위로 조회하는 서비스 클래스입니다.
 * 현재 로그인된 사용자의 이메일을 기반으로 특정 학년에서의 성적 백분위수를 계산합니다.
 */
@Service
...
public class GetStudentPercentInGradeService implements GetStudentPercentInGradeUseCase {
    ...
    public Integer execute(PercentileType percentileType, Integer grade) {
        ...
        Integer studentScore = switch(percentileType) {
            case HIGH -> scorePersistencePort.getStudentHighPercentileByEmailInGrade(email, grade);
            case LOW -> scorePersistencePort.getStudentLowPercentileByEmailInGrade(email, grade);
        };
